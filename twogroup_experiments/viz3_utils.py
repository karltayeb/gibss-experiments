...
def _plot_calibration_on_ax(ax: "plt.Axes", panel_df: pl.DataFrame, color: str | None = None) -> None:
    for series_label in sorted(panel_df.get_column("series_label").unique().to_list()):
...
def render_pip_calibration(
    ...
    theme = base_chart_theme()
    methods = sorted(calibration_summary.get_column("method_display").unique().to_list())
    n_cols = len(methods)
    ...
    if facet_by_simulation:
        simulations = sorted(calibration_summary.get_column("simulation_name").unique().to_list())
...
def _plot_power_fdp_on_ax(
    ...
    *,
    ...
    for trace_label in sorted(panel_df.get_column("trace_label").unique().to_list()):
...
def render_power_fdp_chart(
    ...
    if facet:
        simulations = sorted(visible.get_column("simulation_name").unique().to_list())
        ...
        _plot_power_fdp_on_ax(
            ax, visible, max_fdp=max_fdp, fixed_y_scale=fixed_y_scale, title="Aggregate"
        )
        ax.set_ylabel("Power")
...
def _plot_causal_pip_on_ax(ax: "plt.Axes", panel_df: pl.DataFrame, *, title: str) -> None:
    for method_name in sorted(panel_df.get_column("method").unique().to_list()):
        ...
        if method_df["threshold"].is_null().all():
            ...
            ax.axhline(y=y_val, color=color, linestyle="--", linewidth=1.5, label=label)
        else:
...
def render_causal_pip_chart(
    ...
    if facet:
        simulations = sorted(causal_pip_summary.get_column("simulation_name").unique().to_list())
        n_cols = len(simulations)
        fig, axes = plt.subplots(
            1, n_cols,
            figsize=(theme["width"] * n_cols, theme["height"]),
        ...
        _plot_causal_pip_on_ax(ax, causal_pip_summary, title="Aggregate")
